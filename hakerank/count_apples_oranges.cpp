...
void countApplesAndOranges(unsigned int s, unsigned int t, unsigned int a, unsigned int b, vector<int> apples, vector<int> oranges)
{
    unsigned int count_apples = 0;
    unsigned int count_oranges = 0;
    for (unsigned int i = 0; i < apples.size(); i++)
    {
        if (((apples.at(i) + a) <= t) && ((apples.at(i) + a) >= s))
        {
            count_apples++;
        }
    }
    ...
    {
        if (((oranges.at(i) + b) <= t) && (oranges.at(i) + b) >= s)
        {
            count_oranges++;
        }
    }
